...
struct vote_producer_unfreezeram_subcommand {
   string voter_str;
   string bpname_str;

   vote_producer_unfreezeram_subcommand(CLI::App* actionRoot) {
      auto vote_proxy = actionRoot->add_subcommand("unfreezeram", localized("unfreeze the EOSC which Withdrawal of voting"));
      vote_proxy->add_option("voter", voter_str, localized("The voting account"))->required();
      vote_proxy->add_option("bpname", bpname_str, localized("Receive the dividend bp name"))->required();
      add_standard_transaction_options(vote_proxy);
      ...
      vote_proxy->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ("bpname", bpname_str)
                  ;
         send_actions({create_action({permission_level{voter_str,config::active_name}}, config::system_account_name, N(unfreezeram), act_payload)});  
      });
   }
};
...
struct vote_producer_vote4ram_subcommand {
   string voter_str;
   string bpname_str;
   string amount;
   //string con = "eosio";

   vote_producer_vote4ram_subcommand(CLI::App* actionRoot) {
      auto vote_proxy = actionRoot->add_subcommand("vote4ram", localized("Vote your stake to a bp"));
      vote_proxy->add_option("voter", voter_str, localized("The voting account"))->required();
      ...
      vote_proxy->set_callback([this] {
         ...
         send_actions({create_action({permission_level{voter_str,config::active_name}}, config::system_account_name, N(vote4ram), act_payload)});
      });
   }
};
